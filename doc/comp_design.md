...
- Render Components render loyally based on the `data` prop. They send change requests upward (callbacks). Mobx store decides whether to accept each request and update their data; they may also talk to a remote server over the network.
...
- display style, such as icon/grid/list display for a component that display resources instances like a file explorer.


If mobx store manages data of multiple resource instances, the data of different resource instances should always be keyed by id. Similarly, if mobx store manages operational data of multiple render components of same type, that appear on different places of frontend ui, they should not share data, but should have their own data, keyed by a random/pseudo frontend component id.
